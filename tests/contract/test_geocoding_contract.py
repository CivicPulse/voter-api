...
class TestGeocodeContractResponse:
    ...
    async def test_404_response_has_detail(self, client) -> None:
        ...
        with patch(
            ...
            resp = await client.get(
                "/api/v1/geocoding/geocode?address=99999+Nonexistent+Rd,+Nowhere,+GA+00000"
            )
...
class TestPointLookupContractResponse:
    ...
    async def test_200_response_matches_schema(self, client) -> None:
        ...
        mock_response = PointLookupResponse(
            latitude=33.749,
            longitude=-84.388,
            accuracy=None,
            districts=[],
        ...
        with patch(
            "voter_api.api.v1.geocoding.point_lookup_districts",
            new_callable=AsyncMock,
            return_value=mock_response,
        ):
            resp = await client.get("/api/v1/geocoding/point-lookup?lat=33.749&lng=-84.388")

        # Will return 200 if the endpoint exists and works
