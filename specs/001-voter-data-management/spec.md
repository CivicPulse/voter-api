...
- What happens when the geocoding service is unavailable or rate-limited? The system MUST queue failed geocoding attempts for retry using exponential backoff (initial delay 60 seconds, max 3 retries per address per batch run). After max retries, the address is marked as "geocoding_failed" with the failure reason. The system MUST report the outage without losing progress on already-geocoded records. Failed addresses from a completed batch can be retried by triggering a new batch geocoding run.
...
- What happens when the same address is geocoded with a different provider than the one cached? The system MUST treat each provider's cache independently — a cache hit for Provider A does not satisfy a request targeting Provider B. Each provider maintains its own cache namespace.

### Out of Scope

The following are explicitly **not** part of this project and will be addressed in separate projects:

- Web UI, frontend, SPA, or any browser-based interface
- Map-based visualization or pin-drop coordinate selection UI
- Public-facing voter lookup portals
- Mobile applications

This project delivers: REST API, shared libraries, and CLI only. The API is designed to be consumed by future frontend projects.
...
- **FR-029**: When a new voter file is imported, the system MUST soft-delete voters present in the previous import but absent from the new file by flagging them as "not in latest import" with a timestamp. All data (geocoding, analysis results) for soft-deleted voters MUST be retained.
- **FR-030**: After each import, the system MUST generate an import diff report showing: (1) voters newly added compared to the previous import, (2) voters removed (soft-deleted) compared to the previous import, and (3) voters whose records were updated. This report MUST be available for export and queryable against any historical import snapshot.
...
- **FR-020**: System MUST support multiple geocoding service providers through a pluggable provider architecture. The system MUST allow administrators to configure which provider to use.
- **FR-021**: System MUST cache geocoding responses per provider. When an address is submitted for geocoding and a cached result exists for that exact address from the same provider, the system MUST return the cached result without making an external call. Each provider MUST have its own independent cache so that results from different providers do not conflict.
...
- **FR-011**: System MUST support searching voters by name (first, last, partial match), address, voter ID, registration status, county, district, and precinct. County MUST be a first-class filter available on all query and export operations.
...
- **FR-018**: System MUST authenticate all API requests using **JWT bearer tokens**. Users and services authenticate with credentials (username/password or OAuth2) and receive short-lived JWTs containing role claims (admin/analyst/viewer). JWTs MUST be the sole authentication mechanism. No endpoint — including those serving government-sourced public record data — MUST be accessible without a valid JWT, **except**: the address geocoding (`/geocoding/geocode`), address verification (`/geocoding/verify`), and point-lookup (`/geocoding/point-lookup`) endpoints, which are public to support the Address Lookup tool for anonymous users. These public endpoints are protected by the global rate limit (200 req/min per IP).
- **FR-019**: System MUST validate all input data at the API boundary before processing.
- **FR-022**: System MUST classify data into two sensitivity tiers: (1) **Government-sourced** — data imported directly from the Secretary of State voter file, which is public record but still requires authentication to access; (2) **System-generated** — data produced by this system (validated addresses, manually updated voter information, exact geocoding coordinates from manual entry or field surveys, analysis results), which is considered sensitive and MUST be restricted to authorized roles with elevated permissions.
- **FR-023**: System MUST maintain an audit log of all data access events, recording which authenticated user viewed, queried, or exported which records, with timestamps. Audit logs MUST be immutable and retained independently of the data they reference.
- **FR-024**: System MUST enforce field-level access control based on user role and data sensitivity tier. System-generated sensitive fields (validated addresses, manual geocoding, analysis results) MUST only be accessible to users with analyst or administrator roles.
- **FR-025**: System MUST store each location analysis execution as a timestamped snapshot. Previous analysis runs and their results MUST be retained as historical records. Users MUST be able to query and compare results across analysis runs (e.g., to see how mismatches changed after a redistricting event or re-geocoding).
- **FR-026**: System MUST support manual coordinate entry for a voter via API endpoint and CLI command, recorded with source type "manual" or "field-survey". A map-based pin-drop interface is deferred to a future frontend project.
- **FR-027**: System MUST preserve all geocoding results per voter across all sources (each automated provider and manual entries). Users MUST be able to view and compare results from different sources for the same voter side-by-side.
- **FR-028**: System MUST designate exactly one geocoding result per voter as the "primary" location used for spatial analysis. Administrators MUST be able to change which result is primary (e.g., preferring a manual field-survey result over an automated one).

### Key Entities

- **Voter**: Represents an individual voter record sourced from the Georgia Secretary of State voter file. Key attributes: county, voter registration number (unique identifier), status (ACTIVE/INACTIVE), status reason, last name, first name, middle name, suffix, birth year, residence address (street number, pre-direction, street name, street type, post-direction, apt/unit number, city, zip code), mailing address (street number, street name, apt/unit, city, zip, state, country), registration date, race, gender, last modified date, date of last contact, last party voted, last vote date, voter created date, present_in_latest_import (boolean), last_seen_in_import (import job reference), first_seen_in_import (import job reference), soft_deleted_at (timestamp, null if active). Registered assignments: county precinct (code + description), municipal precinct (code + description), congressional district, state senate district, state house district, judicial district, county commission district, school board district, city council district, municipal school board district, water board district, super council district, super commissioner district, super school board district, fire district, municipality, combo, land lot, land district. Relationships: has zero or more geocoded locations (one per source), falls within zero or more boundaries based on physical location.
- **GeocodedLocation**: Represents a single geocoding result for a voter's residence address from a specific source. A voter may have multiple GeocodedLocations — one per geocoding source (e.g., Census Bureau, Google Maps, manual entry, field survey). Key attributes: latitude, longitude, geocoding confidence score, source type (automated provider name or "manual" or "field-survey"), geocoded timestamp, point geometry, is_primary (boolean — indicates which result is used for spatial analysis). One result MUST be designated as primary for use in district/precinct analysis; the others are retained for comparison. The geocoder MUST reconstruct a full address from the component fields (street number + pre-direction + street name + street type + post-direction + apt/unit + city + state + zip) for automated sources. Relationships: belongs to one voter; a voter has zero or more GeocodedLocations.
- **GeocoderCache**: Represents a cached geocoding response keyed by provider and normalized address. Key attributes: provider name, normalized input address, latitude, longitude, confidence score, raw provider response, cached timestamp. The cache is per-provider so that switching or adding providers does not invalidate or conflict with existing cached results. Cache lookups use exact normalized address match.
...
- **User**: Represents an authenticated human user of the system. Key attributes: user ID, username, hashed password, email, role (administrator, analyst, viewer), active status, created timestamp, last login timestamp. Users authenticate via credentials to receive JWT tokens.
- **AuditLog**: Represents an immutable record of a data access event. Key attributes: log ID, timestamp, actor (user ID from JWT), action (view, query, export, import, analyze, update), resource type (voter, boundary, analysis, etc.), resource identifiers, request metadata (IP, endpoint). Retained independently of referenced data.
...
- **AnalysisRun**: Represents a single execution of the location analysis process. Key attributes: run ID, triggered by (user reference), started timestamp, completed timestamp, status (pending, running, completed, failed), total voters analyzed, match count, mismatch count, unable-to-analyze count, notes (e.g., "post-redistricting re-analysis"). Each run produces a complete snapshot of results. Previous runs are retained as historical records.
- **AnalysisResult**: Represents the outcome of comparing a voter's physical location to their registration within a specific analysis run. Key attributes: analysis run reference, voter reference, determined district(s), determined precinct(s), registered district, registered precinct, match status (match, mismatch-district, mismatch-precinct, mismatch-both, unable-to-analyze), analysis timestamp. Results are immutable once their parent run completes. Historical results from prior runs are preserved indefinitely.
...
- The US Census Bureau Geocoder will be used as the default geocoding provider (free, no API key required for batch processing). The architecture MUST support multiple pluggable geocoding providers (e.g., Google Maps, Mapbox, HERE) to allow switching or adding services. Geocoding responses MUST be cached per provider so that repeated requests for the same normalized address skip the external call. Cache entries do not expire by default but administrators can trigger re-geocoding to refresh results.
...
- The system will initially operate with two Georgia counties' data. The data model MUST treat county as a first-class filter on all queries and operations from day one, supporting statewide scale (all 159 Georgia counties) without schema changes. Full statewide data acquisition will be deferred until the system is validated. The data model will not be hardcoded to Georgia's specific schema, allowing future expansion to other states.
- Export formats are CSV (for spreadsheet tools), JSON (for API consumers), and GeoJSON (for GIS tools).
- User roles will include at minimum: administrator (full access including system-generated sensitive data), analyst (read + analysis including system-generated sensitive data), and viewer (read-only access to government-sourced data only).
- Data is classified into two sensitivity tiers: (1) Government-sourced data (SoS voter file fields) is public record but MUST still require authentication to access via API as a courtesy measure; (2) System-generated data (validated addresses, manual geocoding, field survey coordinates, analysis results) is considered sensitive and restricted to analyst/administrator roles.
...
- Q: What level of PII/data protection is required for voter data? → A: Two-tier model. Government-sourced data (SoS voter file) is public record but all endpoints require authentication as a courtesy. System-generated data (validated addresses, manual geocoding, field survey data, analysis results) is sensitive and restricted to analyst/administrator roles. Audit logging of all data access is required. Field-level access control by role enforced.
- Q: Should analysis results be overwritten or preserved as historical snapshots? → A: Historical snapshots. Each analysis run is stored as a timestamped snapshot with all results retained. Previous runs are preserved indefinitely, enabling comparison across runs (e.g., post-redistricting).
- Q: Single-county, statewide, or progressive multi-county scope? → A: Progressive. Currently two counties' data available; full statewide data is expensive to acquire. Data model MUST support statewide (all 159 GA counties) from day one with county as a first-class filter. Statewide acquisition deferred until system is validated.
- Q: Should the system support manual coordinate entry alongside automated geocoding? → A: Yes — automated + manual via API and CLI. All geocoding results from every source (each automated provider + manual + field survey) MUST be preserved per voter for side-by-side comparison. One result is designated as primary for spatial analysis; administrators can change which is primary. Map-based pin-drop UI deferred to future frontend project.
- Q: How should the system handle voters absent from a new import file? → A: Soft-delete with import diff reporting. Voters absent from the new file are flagged "not in latest import" with timestamp; all data retained. System generates an import diff report (added/removed/updated voters) after each import, queryable against any historical snapshot. Enables tracking voter roll changes over time and lets voters verify their status.
- Q: What is the project scope regarding UI/frontend? → A: API-first. Project delivers REST API, shared libraries, and CLI only. No web UI, frontend, SPA, or browser-based interface. Map-based pin-drop UI deferred to a future separate frontend project. All functionality MUST be accessible via API and CLI.
- Q: What authentication mechanism for the API? → A: JWT bearer tokens only. Users and services authenticate with credentials and receive short-lived JWTs containing role claims (admin/analyst/viewer). JWTs are the sole authentication mechanism. (Corrected from initial hybrid selection.)
