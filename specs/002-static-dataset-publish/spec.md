...
- Q: When mixed filters are provided (e.g., `?boundary_type=congressional&county=Fulton`), should the redirect use boundary_type or fall back to DB? → A: Any county or source filter forces DB fallback, even when boundary_type is also present. Static files contain all boundaries of a type, so county/source-scoped requests cannot be served from them.
- Q: When the manifest cache TTL expires and R2 is temporarily unreachable, what should the API do? → A: Serve stale cached manifest and retry refresh on the next request. Maximizes availability — redirects continue with last-known-good data during transient R2 outages.
- Q: When R2 is enabled but no manifest has been published yet, what should the discovery endpoint return? → A: HTTP 200 with the configured `base_url` (from R2_PUBLIC_URL) and an empty `datasets` list. The base_url is still useful for consumers to know where future datasets will appear. This distinguishes from R2-disabled (where base_url is null).
- Q: How should the API handle redirects to R2 files that have been deleted (stale manifest)? → A: Accept as-is — no server-side detection. Consumer receives a 404 from R2; admin resolves by republishing. Adding HEAD checks per redirect would negate performance benefits. Document as known behavior.
- Q: Where should the `publisher_version` value in the manifest come from? → A: The project's package version from `pyproject.toml` via `importlib.metadata.version("voter-api")`. Automatically maintained, no extra versioning scheme needed.
...
- What happens when a published static file is deleted from object storage but the manifest still references it? The API does not perform per-request validation of R2 files — the consumer receives a 404 from R2. The administrator uses `publish status` to diagnose staleness and resolves it by republishing. This is an accepted tradeoff: adding HEAD checks per redirect would negate the performance benefit of offloading to static files.
...
- **FR-017**: When the GeoJSON endpoint receives any `county` or `source` filter — even if `boundary_type` is also present — the system MUST fall back to serving the response directly from the database. Static files contain complete datasets for a boundary type and cannot serve county- or source-scoped subsets. Only requests with `boundary_type` as the sole filter (or no filters at all) are eligible for redirect.
...
- **FR-019**: The publish command MUST generate and upload a `manifest.json` file alongside the dataset files, containing metadata (storage key, public URL, record count, file size, publish timestamp) for each published dataset. The manifest's `publisher_version` field MUST be populated from the project's package version (`importlib.metadata.version("voter-api")`).
