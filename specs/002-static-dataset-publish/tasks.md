...
- [ ] T025 [US2] Extend publish_datasets() in src/voter_api/services/publish_service.py — when --boundary-type is set: regenerate only that type's file. When --county or --source is set: query DB to identify which boundary types contain matching boundaries, then regenerate only those types' files. Each regenerated file always contains ALL boundaries of its type (county/source scope which types to republish, not what data goes in them). When filters active: skip combined all-boundaries file, fetch existing manifest from R2 via fetch_manifest, merge new dataset entries into existing manifest (preserving entries for types not being republished), upload merged manifest
